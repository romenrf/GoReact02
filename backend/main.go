...
func GetConnection() *sql.DB {
	...
	db, err = sql.Open("sqlite3", "./bdpruebas")
	if err != nil {
		fmt.Println(err)
	}
	return db
...
// Nos permite manejar las peticiones a la ruta /notes con el método GET vía JSON
...
func setupRoutes(r *mux.Router) *mux.Router {
	...
	r.HandleFunc("/users", handlerUsers)

	return r
}

func getRouter(onlyLocalhost bool) *mux.Router {
	var router *mux.Router

	if onlyLocalhost {
		router = mux.NewRouter().StrictSlash(true).Host("localhost").Subrouter()
	} else {
...
func main() {
	...
	onlyLocalhost := flag.Bool("localhost", false, "Accept only request from localhost")
	r := getRouter(*onlyLocalhost)
